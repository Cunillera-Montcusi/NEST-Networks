...
###################################################################################################
Coalescent.exp.Kernel.J<-function(Meta.pool, Js,filter.env,M.dist, D50,id.obs,
                                  m.pool=0.001, m.max=1){
  ...
  Meta<-list("out"=c("m.pool"=m.pool, "Js.max"=max(Js),"Js.min"=min(Js), "D50"=D50, "m.max"=m.max,
                     ...
                     "simp"=vegan::diversity(apply(Meta[,id.obs],1,sum),"simpson"),
                     "inv.simp"=vegan::diversity(apply(Meta[,id.obs],1,sum), "invsimpson")),
  "MetaCom"=MetaCom_t) 
  Meta
}
